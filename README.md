...
- **丰富的聊天体验**:
    ...
    - **性能统计**：鼠标悬停在 AI 回复的详情按钮上，可查看上下文长度(Context)、输出长度(Output)和生成速率(Rate)。
- **对话持久化**:
    - 对话自动保存到浏览器本地存储，刷新页面后对话记录不会丢失。
    - **备份对话**：将完整对话导出为 JSON 文件，方便跨设备迁移或长期存档。
    - **恢复对话**：从 JSON 备份文件恢复对话历史。
- **版本同步**：完整版和轻量版共享相同的存储空间，配置和对话记录在两个版本间同步。
- **高度可定制**:
    - 提供**浅色**和**深色**两种主题模式。
    - **Lite 模式**: 一键切换到简洁模式，禁用动画和毛玻璃效果，适合低性能设备或追求简洁的用户。
...
1.  **下载文件**: 下载整个项目文件夹，包含 `index.html`、`css/` 和 `js/` 目录。
2.  **打开应用**: 在您喜欢的现代浏览器（如 Chrome, Firefox, Edge）中打开 `index.html` 文件。

### 方式二：轻量版本

1.  **下载文件**: 下载 `nchat-lite.html` 以及 `css/` 和 `js/` 目录。
2.  **打开应用**: 在浏览器中打开 `nchat-lite.html` 文件。

> **注意**: 轻量版本移除了图片上传功能，采用更简洁的界面设计，适合只需要文本对话的用户。

## 📁 项目结构

```
nchat/
├── index.html              # 完整版本（支持图片上传、毛玻璃效果）
├── nchat-lite.html         # 轻量版本（纯文本对话、更简洁）
├── ntools.html             # 工具集（待办、二维码、Markdown编辑器等）
├── css/
│   ├── shared.css          # 共享样式（两个版本共用）
│   ├── full.css            # 完整版本特有样式（动画、毛玻璃、图片）
│   └── lite.css            # 轻量版本特有样式
├── js/
│   ├── marked.min.js       # Markdown 解析器
│   ├── purify.min.js       # XSS 防护
│   ├── qrcode.min.js       # 二维码生成
│   └── modules/            # JavaScript 模块（可选）
│       ├── utils.js        # 工具函数
│       └── ui.js           # UI 交互
└── img/
    └── main.png            # 截图
...
3.  **Base URL**: 填入您的 AI 服务提供商的 API 地址 (Base URL)。
...
6.  **System Prompt**: (可选) 输入系统提示词来指导 AI 的行为。
...
- **切换主题**: 点击右上角菜单 → "切换深色/浅色模式" 即可在两种主题间切换。
- **Lite 模式**: 点击右上角菜单 → "Lite 模式" 可切换到简洁模式。在 Lite 模式下：
    - 禁用所有页面动画和过渡效果
    - 移除毛玻璃效果，使用不透明背景
    - 背景变为静态纯色
    - 再次点击 "完整模式" 即可恢复完整效果。
- 设置会自动保存到浏览器本地，刷新页面后不会丢失。

### 聊天交互

- **发送图片**: 点击输入框左侧的 `+` 图标上传图片（最多5张）。（仅完整版本）
...
- **自动保存**: 所有对话记录会自动保存到浏览器本地存储，刷新页面后对话不会丢失。
- **备份对话**: 点击右上角菜单 → "备份对话"，将完整对话导出为 JSON 文件。
- **恢复对话**: 点击右上角菜单 → "恢复对话"，选择之前备份的 JSON 文件即可恢复对话。
- **清除对话**: 点击右上角菜单 → "清除对话"，清空所有对话记录（此操作会同时清除本地存储的对话）。
